...
- **Verify (the stopping condition):** `pnpm test` — exits 0 ⇒ done. "Done" is a claim; this is the proof.
...
```
/goal all workspace tests pass — verified when `pnpm test` exits 0. Stop after 40 iterations, 3 no-progress turns, or $25.
```
...
Progress is measured through the verify command's output — make `pnpm test` print a
