...
> **This is the source of truth.** Every harness adapter in `adapters/` is derived from this
> file. Edit here, then run `scripts/sync-adapters` (or copy by hand) so Claude Code, Codex,
> Copilot, Cursor, Antigravity, and VS Code all stay in agreement. `AGENTS.md` is the format the
> most tools read natively (Codex, Antigravity, opencode, and others); the rest get a thin
> generated copy under their own filename (`CLAUDE.md`, `GEMINI.md`,
> `.github/copilot-instructions.md`, `.cursor/rules/`).

Keep this file under ~250 lines. It must survive context compaction, so every line earns its
place. Project-specific facts go in the `## Project` block at the bottom; everything above it is
...
<!-- Fill these in per repo. This is the only section that should differ between projects. -->

- **Stack:** <language(s), framework(s), runtime>
- **Vector store / graph / DB:** <e.g. pgvector; no graph; Postgres> — needed by the RAG harness
- **Models:** <generation model, judge model (use a *different* family for judging), embedder>
- **Build:** `<command>`
- **Test:** `<command>`
- **Lint / format:** `<command>`
- **Conventions:** <anything load-bearing — "we use persistent user IDs, never email, to correlate
  accounts"; "all money math in integer cents"; etc.>
